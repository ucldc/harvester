...
class Registry(object):
    ...
    def __init__(self, url_base=url_base, url_api=''.join((url_base, api_path))):#, object_type='Collection', filter=None):
        self.url_base = url_base
    ...
    def resource_iter(self, object_type, filter=None):
        ...
        '''
        if not object_type in self.endpoints:
            raise ValueError('Unknown type of resource {0}'.format(object_type))
        path = self.endpoints[object_type] if not filter else self.endpoints[object_type] + '?' + filter
